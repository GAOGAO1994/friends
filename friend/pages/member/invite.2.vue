		...
		<view class="padding-h">			
			...
			<view v-if="tabIndex == 0" class="invite-wrap">
				<view class="item" v-for="(invite, index) in invites" :class="index===invites.length-1?'last':''">
					<view class="left">
						<image class="avatar" mode="scaleToFill" :src="invite.relativeMmeber.avatarUrl"></image>
					</view>
					<view class="right">
						<view class="title-wrap">
							<text class="nickname">{{invite.relativeMmeber.nickname}}</text>
							<text class="date">{{invite.date}}</text>
						</view>
						<view class="sub-title-wrap">
							<view class="btn-wrap">
								<text class="status label" v-if="invite.member.id == member.id">我发出的邀请</text>
								<text class="status label" v-else>我收到的邀请</text>
							</view>
							<view v-if="invite.member.id == member.id" class="btn-wrap">
		...
		methods:{
			...
			handler:function(data){
				...
				data.invites.forEach(item =>{
					...
					if(item.member.id == this.member.id){
						item.relativeMmeber = item.targetMember;
					}else{
						item.relativeMmeber = item.member;
					}
					if(item.kind == 'invite'){
...
<style>
	...
	.invite-wrap .item .left{
		display: flex;
		flex-direction: row;
		align-items: center;
	}
	.invite-wrap .item .right{
		height: 120upx;
		display: flex;
	...
	.invite-wrap .item .title-wrap{
		display: flex;
		flex-direction: row;
		justify-content: space-between;
	}
	.invite-wrap .item .sub-title-wrap{
		display: flex;
		flex-direction: row;
		justify-content: space-between;
	...
	.invite-wrap .item .title-wrap .nickname{
		
	}
	.invite-wrap .item .title-wrap .status{
