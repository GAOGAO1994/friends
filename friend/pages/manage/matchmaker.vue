		...
		<view class="content-wrap" v-if="isAdmin">
			...
			<view class="padding-h">
				<view class="search-wrap">
					<view class="input-row">
						<m-input class="m-input" type="text" v-model="mobile" placeholder="输入手机号"></m-input>
					</view>
			...
			<view class="list padding-l">
				<view class="item border" v-for="(member,index) in list" @click="viewMember(member)">
					...
					<view class="right">
						<button class="male" @click.stop="check(member)">{{member.memberStatus == '1'?'取消':'通过'}}</button>
					</view>
				</view>	
			</view>
		</view>
		...
		methods:{
			...
			update:function(member){
				...
				uni.request({
					...
					data:{
						id:member.id,
						memberStatus:(member.memberStatus == '1')?'0':'1'
					},
					success(res) {
						if(that.$http.success(res)){
							member.memberStatus = (member.memberStatus == '1')?'0':'1';
						}
					}
				})
			},
